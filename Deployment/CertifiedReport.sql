...
set @subject = ''Certified Report ''

declare @query varchar(2000)
set @query = ''SET NOCOUNT ON; 
		exec rpt.usp_CertifiedReport @param = '''''' +  @param + '''''', @paramType = '''''' + @paramType + 
				'''''',@fromDate = '''''' + convert(varchar,@fromdate,101) + '''''',@todate= '''''' +  convert(varchar,@todate,101) + ''''''''

declare @fileName varchar(2000)
set @fileName = ''CertifiedReport.xml''

declare @body varchar(max)
set @body = ''This is an automated message.
Attached is the Certified Report from '' + convert(varchar(12),@fromdate) + '' to '' + convert(varchar(12),@todate) 
...
update Report set jobsql = @sql where ReportName = 'CertifiedReport'